...
include '../action.php';
            ...
            $sql = "SELECT * FROM management WHERE otp ='".secure($_POST['otp'])."'";
            $query = mysqli_query($conn,$sql);
            $num = mysqli_num_rows($query);
            if($num==1){

                
                    $sql1 ="UPDATE management SET managementPassword = '".secure(md5($_POST['password']))."' WHERE otp= '".secure($_POST['otp'])."' ";
                    $query = mysqli_query($conn,$sql1);
                    if($query){

                    $sql2 ="UPDATE management SET otp = '0' WHERE otp = ".secure($_POST['otp'])."";
