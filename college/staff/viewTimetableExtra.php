...
$stmt = isset($_GET['classID']) ? "where modules.moduleCourseID =   {$_GET['classID']}" : "";

$sql = "SELECT DISTINCT (lessonStart) as time from extralessons

inner join modules on   modules.moduleID = extralessons.subjectID {$stmt} order by lessonStart";
        ...
        foreach ($week_days as $day) {
            ...
            if (array_key_exists($day, $classes)) {
                ...
                <td>
                    ...
                    <?php
                    if (!empty($_SESSION['adminLevel']) === '4') { ?>
                        <a onclick="return confirm('Are you sure you want to delete this?')" class="pull-right"
                           href="back/deleteTimetableExtra.php?id=<?php
                           echo $row['lessonID']; ?>">
                            <button class=" btn-xs" style="border: none;"><i class="fa fa-trash "
...
where extralessons.tutorID = '" . $_SESSION['adminID'] . "'  order by lessonStart ASC";
    ...
    while ($t = mysqli_fetch_array($rest)) {
        $sql1 = "select * from extralessons inner join modules on modules.moduleID = extralessons.subjectID where   lessonStart = '" . $t['time'] . "'";
        //print_r($sql);exit;
        $res = mysqli_query($conn, $sql1);
        ?>
