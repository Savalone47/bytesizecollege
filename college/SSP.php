 ...
 * @copyright DATE 6/2/2021 - 11:12 AM
 */
...
class SSP
{
    ...
    static function filter(array $request, array $columns, array &$bindings): string
    {
        ...
        if (count($columnSearch)) {
            $where = $where === '' ? implode(' AND ', $columnSearch) : $where . ' AND ' . implode(
                    ' AND ',
                    $columnSearch
                );
        }
     ...
     *
     * @param array $request Data sent to server by DataTables
     * @param array|PDO $conn PDO connection PDO or connection parameters array
     * @param string $table SQL table to query
     * @param string $primaryKey Primary key of the table
     * @param array $columns Column information array
     * @return array          Server-side processing response array
     */
    static function simple(array $request, $conn, string $table, string $primaryKey, array $columns): array
    {
        $bindings = array();
        $db = self::db($conn);
    ...
    static function sql_connect(array $sql_details): PDO
    {
        try {
            ...
            self::fatal(
                "An error occurred while connecting to the database. " . "The error reported by the server was: " . $e->getMessage(
                )
            );
        }
     ...
     */
    static function sql_exec(PDO $db, $bindings, string $sql = null): array
    {
        // Argument shifting
        if ($sql === null) {
            $sql = $bindings;
     ...
     */
    static function _flatten($a, string $join = ' AND ')
    {
        if (!$a) {
            return '';
        } else {
